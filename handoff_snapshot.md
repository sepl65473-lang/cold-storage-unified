...
1.  **Telemetry Path**: IoT Devices (or [simulate_device.py](file:///d:/Cold%20storage%20web%20and%20app/backend/simulate_device.py)) publish JSON to MQTT -> [mqtt/client.py](file:///d:/Cold%20storage%20web%20and%20app/backend/app/mqtt/client.py) (Bridge) consumes -> Dispatches [ingest_telemetry](file:///d:/Cold%20storage%20web%20and%20app/backend/app/workers/ingest.py#26-37) Celery Task -> Written to TimescaleDB Hypertable (`sensor_readings`) + Published to Redis Channel.
2.  **Live Updates**: Redis `telemetry_stream` Channel -> [services/stream.py](file:///d:/Cold%20storage%20web%20and%20app/backend/app/services/stream.py) ([StreamManager](file:///d:/Cold%20storage%20web%20and%20app/backend/app/services/stream.py#12-64)) -> WebSocket Broadcast to connected Frontend clients (via [useDevices](file:///d:/Cold%20storage%20web%20and%20app/web/src/hooks/useDevices.ts#28-93) hook).
...
### Active Models
- **[User](file:///d:/Cold%20storage%20web%20and%20app/backend/app/models/user.py#44-66) / [Organization](file:///d:/Cold%20storage%20web%20and%20app/backend/app/models/user.py#24-35)**: RBAC (Superadmin, Admin, Operator, Viewer).
- **[Device](file:///d:/Cold%20storage%20web%20and%20app/web/src/hooks/useDevices.ts#6-27)**: Metadata, location (lat/lng), status (online/offline).
- **[SensorReading](file:///d:/Cold%20storage%20web%20and%20app/backend/app/schemas/reading.py#13-25)**: Hypertable partitioned by `time`. Temperature, Humidity, Battery, Solar.
- **[Alert](file:///d:/Cold%20storage%20web%20and%20app/backend/app/models/alert.py#28-43)**: Triggered on threshold violations (e.g., Temp > 8C).
- **[OTAUpdate](file:///d:/Cold%20storage%20web%20and%20app/backend/app/models/ota.py#46-67) / [FirmwareRelease](file:///d:/Cold%20storage%20web%20and%20app/backend/app/models/ota.py#28-44)**: Tracking version rollouts.
...
### Required [.env](file:///d:/Cold%20storage%20web%20and%20app/backend/.env) Schema (Backend)
```env
